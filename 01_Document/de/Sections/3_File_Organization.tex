...
\begin{itemize}
  \item Der Zugang zum VT-Server erfolgt über Ihren Webbrowser durch Eingabe der Webadresse
        ...
        verwaltet.
  \item Die File-Server der anderen Hauptabteilungen sind ebenfalls mittels
        Browser oder Win\-dows-Explorer erreichbar. Die Zugriffsberechtigung wird von der
        IT-Abteilung des IWTs verwaltet.
\end{itemize}

\subsection{Datentypen und Speicher-Strategie großer Daten}
...
\end{itemize}
Projektrelevante Daten sind im jeweiligen Projektordner abzulegen, und bei Experimenten, die im Elektronischen Labor-Notizbuch (ELN) dokumentert sind, muß im Projektordner jeweils ein ELN-Protokoll als PDF hinterlegt werden.
Sehr große Primärdaten können nicht auf dem ELN gespeichert werden, aber die Herkunft dieser Daten wird dort zusammen mit Informationen über den Speicherort (z.B. wo genau sie auf einem Dateiserver liegen) erklärt. Dabei gelten folgende Regeln:
\begin{itemize}
  \item Daten $<$ 100 MByte werden im ELN gespeichert und dokumentiert
  \item Alle Daten $>$ 100 MByte werden auf dem File-Server (in Ausnahmen auf USB-Platten) gespeichert und im
        ELN dokumentiert
\end{itemize}
Um eine High-Speed-Aufnahme mit mehreren Gigabyte Datengröße denoch im ELN zu dokumentieren, kann eine hochkomprimierte mp4-Datei aus den Originalaufnahmen erzeugt und diese dann hochgeladen werden.

Mit großen Datenmengen (ab 5 GB) ist besondere Umsicht geboten; siehe z.B. die 10. Regel bei Hart et al. \cite{hart:2016:PLOSComputBiol}. Zusätzlich zu den zwei Regeln für das ELN gibt es je nach Größe der Daten verschiedene Speicher-Strategien, dargestellt in Abb.\ref{fig:storage-strategy}.
\begin{figure}[H]
  \vspace{-1em}
  %Das yEd-File ist hier: /home/riefler/Daten/0/Wissenschaft/Uni+IWT/Computing/IWT-EDV/EDV-Struktur/IT-Ausschuss/DataStorageStrategy-Unterausschuss
  \includegraphics[width=\linewidth]{Figure01_DataStorageStrategy-Deutsch.png}
%  \includegraphics[width=18cm]{Figure01_DataStorageStrategy-Deutsch.png}
  \caption{Pfade der Speicher-Strategie}
  \label{fig:storage-strategy}
\end{figure}
Dabei muss sorgfältig abgewogen werden, ob sich die Daten relativ einfach nochmals erzeugen lassen. Das ist z.B. bei Simulationen prinzipiell der Fall, wenn sämtliche Input-Para\-meter, das komplette Software-Paket sowie möglichst auch das zugrundeliegende Betriebssystem mit abgespeichert wird (ansonsten kann es beim Nachrechnen von Simulationen zu Problemen kommen). Aufwändige Experimente können demgegenüber nicht so einfach wiederholt werden. Deshalb sind große Datenmengen aus Experimenten prinzipiell als wertvoller einzuschätzen im Vergleich mit Simulationsdaten. Der Hintergrund für die sorgsame Entscheidung, welche Daten gespeichert werden, ist die immer größer werdende Menge an erzeugten Daten insbesondere bei Simulationen auf High Performance Computer (HPC) z.B. innerhalb des NHR-Verbunds (Nationale Hochleistungsrechner). Die Datenmenge einer transienten Simulation liegt häufig bei weit über einem Terrabyte. Daten $>$ 1 TByte werden deshalb auf USB-Festplatten oder nach Absprache mit der IT auf einem Fileserver gespeichert, wobei ihr Inhalt wie oben beschrieben im ELN dokumentiert wird.

Die Aufbewahrungsfrist von Daten beträgt 10 Jahre, danach werden alle Daten von der IT gelöscht. Der Verantwortliche für die Daten (d.h. die Führungskraft) bekommt ein halbes Jahr zuvor eine Nachricht, das die Daten gelöscht werden. Wenn sie oder er sich zuvor meldet, werden diese Daten für eine zu bestimmende Zeit über die 10 Jahre hinaus auf dem Server belassen. Die für Publikationen wichtige Daten (Unterordner `Publications' im jeweiligen Projektordner) bleiben erhalten. Werden die Publikationsdaten auf einem Repositorium hinterlegt (z.B. auf zenodo.org), können auch diese auf dem IWT-Ablageort gelöscht werden. Dabei muss jedoch im Publikationsverzeichnis ein entsprechender Hinweis/Link auf das Repositorium hinterlegt werden, um die Daten weiterhin auffindbar zu machen.

Zuletzt soll hier noch angemerkt werden, dass es unbedingt erforderlich ist \textbf{alle Daten aus externen Quellen}, z.B. von einem USB-Stick, manuell auf Viren zu prüfen, bevor dann Daten verwendet oder auf der Festplatte gespeichert werden. Darüber hinaus gehen auf USB-Sticks Daten häufig verloren, wenn diese einige Zeit keinen Strom bekommen, d.h. diese sollten auf keinen Fall zur Datenspeicherung genutzt werden.
...
\noindent Wenn Sie Ihre (Mess-)Daten in ein Format mit den
oben genannten Eigenschaften exportieren, bewahren Sie eine Kopie im
...
\subsubsection{Täglich generierte Daten}\label{ssc:daily-data}
...
\begin{itemize}
  \item LFM (LaborFürMikrozerspanung)
  \item MSK (Metallzerstäubung und SprühKompaktierung)
  \item RST (ReaktiveSprühTechnik)
  \item SM (StrukturMechanik)
  \item WB (WärmeBehandlung)
\end{itemize}
...
für alle Ihre Roh- und Primärdaten ist: \textbf{\underline{Alle Daten müssen bzgl.
...
\begin{itemize}
  \item[$\rightarrow$] \textbf{Ergebnisdokumentation:}
    ...
    \end{itemize}
\end{itemize}
Finanzierungsquellen (\texttt{<Finanzierung>}) sind z.B.: DFG, AiF, BMFTR, Industrie, ERC, etc. Zuletzt wird der Verzeichnisname für veröffentlichte Arbeiten in einem Projekt zusammengefügt aus diesen Komponenten: \\
\texttt{<Jahr Projektstart>\_<Abteilung>\_<Projektbezeichnung>\_<Familienname> }.
Siehe \ref{app:dummy-paper} im Anhang für ein Beispiel.
%sowie die beiden hier angeführten Beispiele:
%\begin{itemize}
...
%\end{itemize}
%Die Dopplung von MPS und RST ist auf einen gemeinsamen Dateiserver zurückzuführen, auf dem in Zukunft die %Daten aller IWT-Abteilungen gespeichert werden sollen.

\paragraph{Papers und Dissertationen}

\noindent In Absprache mit allen IWT-Abteilungen gibt es ein verpflichtendes Schema für alle Veröffentlichungen. Dieses Verzeichnisbenennungsschema hängt ab von der jeweiligen Abteilung, der Finanzierungsquelle und dann dem entsprechenden Projektnamen (siehe oben für die Definition von \texttt{<Projektname>}).
...
\noindent Texte und Daten eines Papers werden z.B. in diesem Verzeichnis gespeichert:\\
\texttt{/File-Server/<Abteilung>/Projekte/<Förderung>/<Projektname>/Paper/} \\
...
abgelegt:
\\
\texttt{/File-Server/<department>/Projekte/<funding>/<project name>/Dissertation/} \\
mit einem Verzeichnisnamen wie z.B.:
\\
...
\texttt{"05\_ProgramSources"}, \texttt{"07\_Supplement"} und \texttt{"08\_Misc"} noch vorhanden sein, aber ohne Einträge. Die Änderungen am Manuskript aufgrund des Revision-Prozesses können durch Anlegen von Unterverzeichnissen \texttt{"01\_Manuscript/01\_Submission"} sowie \linebreak
\texttt{"01\_Manuscript/02\_Revision"} samt Speichern der jeweiligen Manuskript- und Rebbutal-Datei nachvollziehbar gemacht werden.
...
\noindent Poster oder Präsentationen werden entsprechend Tabelle \ref{table:poster-directory-structure} gespeichert unter: \\
\texttt{/File-Server/<Abteilung>/Projekte/<Förderung>/<Projektname>/Poster/} \\
\texttt{/File-Server/<Abteilung>/Projekte/<Förderung>/<Projektname>/Präsentationen/} \\
%
Die Verzeichnisstruktur ist ähnlich wie bei den Papers (siehe Tabelle \ref{table:poster-directory-structure}), d.h. Sie müssen hier wieder alle verwendeten Mess- oder Simulationsdaten speichern. Einige Verzeichnisse haben abweichende Namen und einen anderen Inhalt, wie z.B. \texttt{"01\_Poster+Abstrakt"}, das Poster und Abstrakt der Konferenz enthält. Falls Sie eine mündliche Präsentation gehalten haben, heißen die ersten beiden Verzeichnisse \texttt{"00\_FinalPresentation"} und \linebreak
\texttt{"01\_Praesentation+Abstrakt"}. Bei Änderungen der Reihenfolge von Abbildungen oder Tabellen müssen diese entsprechend neu durchnummeriert werden, wofür ein oben erwähntes Tool existiert zur automatischen Durchnummerierung.

\begin{table}[H]
  \caption{%
  Die Daten jedes Beitrags müssen in acht Unterverzeichnissen gespeichert
